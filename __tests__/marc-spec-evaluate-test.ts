import * as assert from 'assert';
import { parseMarcSpec, MARCSpec } from '../lib/marc-spec';
import { base64RecordAsByteArray, extractFields } from '../lib/marc-parser';
import { buildTree } from '../lib/marc-spec-evaluate';


test('MarcSpec evaluate', () => {
    const marc_data = "\nH3hFeHRyYSBpbmZvcm1hdGlvbh4gIB9jMTYwNDk0MB9kMTYwNTAyNx4d\n";

    const record = base64RecordAsByteArray(marc_data);

    const specs: [string, any][] = [
        ['020[0]$a', [['978-91-7694-778-4']]],
        ['020[0]$a/1-2', [['78']]],
        ['020[1]$a/8-#', [['778-5']]],
        ['020', [['978-91-7694-778-4'], ['91-7694-778-5']]],
        ['LDR/9', [['a']]],
        ['LDR', [['01923nam a22003017a 4500']]],
        ['008[1]', []],
        ['LDR[1]', []],
        ['5..$a{^1=\\7}', [['09123600'], ['09123601']]],
        ['5..$a/0-4{$a/0=\\M|$a/0=\\A|\\I=$a/0|\\9=$a/1}{$a/7!=\\1}', [['Melod'], ['Inneh'], ['Annat'], ['09123']]],
        ['5..$a/1-5{$a~\\är}', [['nnehå'], ['nnat ']]],
        ['5..[0]$a/0-1', [["Me"]]],
        ['5..{$a=[1]$a[0]}', [["09123601"]]],
        ['5..{[1]$a=$a}', [["09123601"]]],
        ['5..{[1]}', [["09123600"], ["09123601"]]],
        ['952$d$l$v', [["2010-01-05", "4", "296.00"]]],
        ['020[0]$a/#-2', [["8-4"]]]
    ];

    const t = [];
    let subscribers = [];

    for (const s0 of specs) {
        const marcSpec = parseMarcSpec(s0[0]);
        if (marcSpec instanceof MARCSpec) {
            const t0 = buildTree(marcSpec);
            t.push(t0);
            subscribers = subscribers.concat(t0.subscribers);
        } else {
            console.log(JSON.stringify(marcSpec));
        }
        assert.ok(marcSpec instanceof MARCSpec);
    }

    extractFields(record, subscribers);

    for (let i = 0; i < specs.length; i++) {
        assert.deepStrictEqual(t[i].evaluate, specs[i][1]);
    }

});
